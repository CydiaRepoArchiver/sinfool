...
while ((n > -1))
do
r=$(echo $(plutil -key patches -key $n -key title "$p") | tr -dc [:alnum:])
mkdir $S/$r
b=$(plutil -key patches -key $n -key applicationIdentifier "$p")
...
echo "Name: $(plutil -key patches -key $n -key title "$p")" >> $S/"$r"/control
...
plutil -key patches -key $n -key description "$p" | while read v
do
echo " $v" >> $S/"$r"/control
done
...
make -C $S/"$r" package
mv $S/"$r"/packages/* sinfool
n=$((n+1))
test "$(plutil -key patches -key $n -key title "$p" 2> /dev/null)" || n=-1
done
